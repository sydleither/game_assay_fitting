...
class ODEModel:
    def __init__(self, **kwargs):
        ...
        self.numericalStabilisationB = kwargs.get(
            "numericalStabilisationB", False
        )  # Whether to apply numerical stabilisation
        self.suppressOutputB = kwargs.get(
    ...
    def Simulate(self, treatmentScheduleList, **kwargs):
        ...
        )  # Time resolution to return the model prediction on
        ...
        self.max_step = kwargs.get(
            "max_step", self.max_step
        )  # Maximum step size permitted by solver
        self.successB = False  # Indicate successful solution of the ODE system
        self.suppressOutputB = kwargs.get(
            "suppressOutputB", self.suppressOutputB
        )  # If true, suppress output of ODE solver (including warning messages)
        self.numericalStabilisationB = kwargs.get(
            "numericalStabilisationB", self.numericalStabilisationB
        )  # Whether to apply numerical stabilisation
        ...
        if self.resultsDf is None or treatmentScheduleList[0][0] == 0:
            ...
            currStateVec = [self.resultsDf[var].iloc[-1] for var in self.stateVars] + [
                self.resultsDf["DrugConcentration"].iloc[-1]
            ]
        resultsDFList = []
        ...
        for intervalId, interval in enumerate(treatmentScheduleList):
            ...
            if self.suppressOutputB:
                with stdout_redirected():
                    solObj = scipy.integrate.solve_ivp(
                        self.ModelEqns,
                        y0=currStateVec,
                        t_span=(tVec[0], tVec[-1] + self.dt),
                        t_eval=tVec,
        ...
        if len(resultsDFList) > 0:
            ...
            resultsDf = pd.DataFrame(
                {
                    "Time": tVec,
                    "DrugConcentration": np.zeros_like(tVec),
                    **dict(zip(self.stateVars, np.zeros_like(tVec))),
                }
            )
        # Compute the fluorescent area that we'll see
        resultsDf["TumourSize"] = pd.Series(
            self.RunCellCountToTumourSizeModel(resultsDf), index=resultsDf.index
        )
    ...
    def Simulate_AT1(
        ...
        while (currInterval[1] <= t_end + intervalLength) and (currCycleId < nCycles):
            ...
            if self.resultsDf.TumourSize.iat[-1] < v_min:  # Withdraw treatment below a certain size
                ...
                self.resultsDf.TumourSize.iat[-1] < (1 - atThreshold) * refSize
            ):  # Reduce dose if sufficient shrinkage
                if mode == "original":
                    dose = max(
                        (1 - doseAdjustFac) * dose, 0
                    )  # Adjustment as proposed in Enriquez-Navas et al (2015)
                else:
                    dose = max(1 / doseAdjustFac * dose, 0)
            elif (
                self.resultsDf.TumourSize.iat[-1] > (1 + atThreshold) * refSize
            ):  # Increase dose if excessive growth
                if mode == "original":
                    dose = min(
                        (1 + doseAdjustFac) * dose, self.paramDic["DMax"]
                    )  # Adjustment as proposed in Enriquez-Navas et al (2015)
                else:
    ...
    def Simulate_AT2(
        ...
        while (currInterval[1] <= t_end + intervalLength) and (currCycleId < nCycles):
            ...
            prevSizesList[-1] = self.resultsDf.TumourSize.iloc[
                -1
            ]  # AT2 uses the 2 time steps to decide dose
    ...
    def Simulate_AT50(
        ...
        D_Star=None,
        D_min=0,
        intervalLength_on=1.0,
        intervalLength_off=None,
        t_end=1000,
        nCycles=np.inf,
        t_span=None,
        solver_kws={},
        ...
        currInterval = [t_span[0], t_span[0] + intervalLength]
        refSize = (
            self.paramDic.get("scaleFactor", 1) * np.sum(self.initialStateList)
            if refSize is None
            else refSize
        )
        ...
        while (currInterval[1] <= t_end + intervalLength) and (currCycleId < nCycles):
            ...
            if (
                self.resultsDf.TumourSize.iat[-1] < (1 - atThreshold) * refSize
            ):  # Withdraw treatment below a certain size
                dose = D_min
    ...
    def Simulate_LongTermAssay(self, treatment_schedule, seeding_density=5e5, solver_kws={}):
        ...
        for cycle_id, cycle in enumerate(treatment_schedule):
            ...
            if cycle_id > 0:
                ...
                curr_resistance_fraction = (
                    self.resultsDf["R"].iloc[-1] / self.resultsDf["TumourSize"].iloc[-1]
                )
                ...
                self.resultsDf["TumourSize"].iloc[-1] = (
                    self.resultsDf["S"].iloc[-1] + self.resultsDf["R"].iloc[-1]
                )
    ...
    # =========================================================================================
    # Simulate a long term assay, where we passage the cells at every interval
    def Simulate_LongTermAssay_PKill(
        self, treatment_schedule, seeding_density=5e5, passaging_loss=0.8, solver_kws={}
    ):
        """
        Simulate a long term assay, where we passage the cells at regular intervals, reseeding them at
        a given density.
    ...
    def Trim(self, t_eval=None, dt=1):
        t_eval = np.arange(0, self.resultsDf.Time.max() + dt, dt) if t_eval is None else t_eval
        tmpDfList = []
        ...
        **kwargs,
        ...
        drugConcentrationVec = utils.TreatmentListToTS(
            treatmentList=utils.ExtractTreatmentFromDf(
                drug_data_df, timeColumn="Time", treatmentColumn="DrugConcentration", mode="post"
            ),
            tVec=drug_data_df["Time"],
        )
        drugConcentrationVec[drugConcentrationVec < 0] = 0
        drugConcentrationVec = np.array(
            [x / (np.max(drugConcentrationVec) + 1e-12) for x in drugConcentrationVec]
        )
        ...
        if kwargs.get("saveFigB", False):
            plt.savefig(
                kwargs.get("outName", "modelPrediction.png"), orientation="portrait", format="png"
            )
            plt.close()
