...
def residual(params, x, data, model, model_to_observation_map, solver_kws={}, residual_kws={}):
    ...
    currSolver_kws["numericalStabilisationB"] = (
        max_step < begin_stabilisation_threshold * solver_kws.get("max_step", np.inf)
    ) or model.numericalStabilisationB
    while not converged:
        ...
        if verbose and (not converged or currSolver_kws["numericalStabilisationB"]):
            print(
                [
                    max_step,
                    currSolver_kws["numericalStabilisationB"],
                    np.any(model.solObj.y < 0),
                    model.solObj.status,
                    model.solObj.success,
                    model.errMessage,
                ]
            )
        ...
        if max_step < 1e-3: 
            print("Aborting: max_step too small. System is likely divergent or overly stiff.")
            break
    # Turn off numerical stabilisation once we've been able to increase the time step above the threshold again
    if not currSolver_kws["numericalStabilisationB"]:
        model.numericalStabilisationB = False
    ...
    for model_feature in model_to_observation_map:
        ...
        f = scipy.interpolate.interp1d(
            model.resultsDf.Time, model.resultsDf[model_feature], fill_value="extrapolate"
        )
        modelPrediction = f(t_eval)
        scale_dict = residual_kws.get("residual_scale", {model_feature: 1.0})
...
# ====================================================================================
def residual_multipleConditions(
    params, x, data, model, model_to_observation_map, split_by, solver_kws={}, residual_kws={}
):
    tmpList = []
    for condition in data[split_by].unique():
        ...
        tmpList.append(
            residual(
                ...
                currData,
                model,
                model_to_observation_map,
                solver_kws=solver_kws,
                residual_kws=residual_kws.get(condition, {}),
            )
        )
...
# ====================================================================================
def residual_multipleTxConditions(
    params, x, data, model, model_to_observation_map, solver_kws={}, residual_kws={}
):
    return residual_multipleConditions(
        params,
        x,
        data,
        model,
...
def prepare_data(data_df, specs_dic, restrict_range=True, average=True, average_by="Time"):
    ...
    for filter_name, filter_value in specs_dic.items():
        ...
        if filter_name in cols_available:
            ...
            )  # Users can specify one or multiple possible values. If only one value is specified, convert to list here
            training_data_df = training_data_df[
                np.isin(training_data_df[filter_name], filter_value_list)
            ].copy()
    ...
    if restrict_range and specs_dic.get("TimeRange", [-np.inf])[0] >= 0:
        ...
        training_data_df = training_data_df[
            ...
            & (training_data_df.Time <= training_range[1])
        ].copy()
        training_data_df["Time"] -= training_range[0]
...
def load_fit(
    modelName,
    fitId=0,
    file_name_model=None,
    fitDir="./",
    model=None,
    load_bootstraps=False,
    file_name_bootstraps=None,
    **kwargs,
):
    ...
    if load_bootstraps:
        file_name_bootstraps = (
            "bootstraps_fit_%d.csv" % (fitId)
            if file_name_bootstraps is None
            else file_name_bootstraps
        )
...
def generate_fitSummaryDf(fitDir="./fits", identifierName=None, identifierId=1, alpha=0.95):
    ...
    """
    fitIdList = [
        int(re.findall(r"\d+", x)[0]) for x in os.listdir(fitDir) if x.split("_")[0] == "fitObj"
    ]
        ...
        tmpDicList.append(
            {
                **identifierDic,
                "FitId": fitObj.fitId,
                "ModelName": fitObj.modelName,
                "AIC": fitObj.aic,
                "BIC": fitObj.bic,
                "RSquared": fitObj.rSq,
                "Success": fitObj.success,
                "Message": fitObj.message,
                "NumericalStabilisation": fitObj.numericalStabilisation,
                ...
                **dict(
                    [
                        (
                            x + "_ci",
                            t.ppf((1 + alpha) / 2.0, fitObj.ndata - fitObj.nvarys)
                            * (
                                fitObj.params[x].stderr
                                if fitObj.params[x].stderr is not None
                                else np.nan
                            ),
                        )
                        for x in fitObj.params.keys()
                    ]
                ),
            }
        )
...
# ====================================================================================
def plot_parameter_spider_plot(
    params,
    axes_dict,
    color="grey",
    alpha=0.8,
...
def perform_bootstrap(
    ...
    n_bootstraps=5,
    shuffle_params=True,
    prior_experiment_df=None,
    model_kws={},
    residual_fun=residual,
    model_to_observation_map={"TumourSize": "Confluence"},
    varyICs=None,
    split_by=None,
    show_progress=True,
    plot_bootstraps=False,
    plot_kws={"ylim": None, "palette": None},
    outName=None,
    **kwargs,
):
    """
    ...
    plot_kws["ylim"] = (
        1.3 * np.max([fitObj.data[x].max() for x in model_to_observation_map.values()])
        if plot_kws["ylim"] is None
        else plot_kws["ylim"]
    )
    plot_kws["palette"] = (
        sns.color_palette("pastel", n_colors=n_bootstraps)
        if plot_kws["palette"] is None
        else plot_kws["palette"]
    )
    if plot_bootstraps:
        fig, ax_list = plt.subplots(
            1, len(model_to_observation_map), figsize=(len(model_to_observation_map) * 8, 6)
        )

    # 1. Perform bootstrapping
    parameterEstimatesMat = np.zeros(
        (n_bootstraps, nvarys + 1)
    )  # Array to hold parameter estimates for CI estimation
    for bootstrapId in tqdm(np.arange(n_bootstraps), disable=(show_progress == False)):
        ...
        if split_by is None:
            for observed_feature in model_to_observation_map.values():
                ...
                tmpDataDf.loc[np.isnan(tmpDataDf[observed_feature]) == False, observed_feature] -= (
                    fitObj.residual
                )  # Obtain the model prediction
                tmpDataDf.loc[np.isnan(tmpDataDf[observed_feature]) == False, observed_feature] += (
                    np.random.normal(loc=0, scale=np.sqrt(residual_variance), size=fitObj.ndata)
                )
        else:
            ...
            for condition in tmpDataDf[split_by].unique():
                for variable, observed_feature in model_to_observation_map.items():
                    ...
                    curr_data_selection = (np.isnan(tmpDataDf[observed_feature]) == False) & (
                        tmpDataDf[split_by] == condition
                    )
                    curr_condition_index_end += tmpDataDf[curr_data_selection].shape[0]
                    curr_residual = fitObj.residual[
                        curr_condition_index_start:curr_condition_index_end
                    ]
                    curr_residual_scale = residual_scale.get(variable, 1.0)
                    curr_residual_var = (
                        np.sum(np.square(curr_residual * curr_residual_scale)) / fitObj.nfree
                    )
                    tmpDataDf.loc[curr_data_selection, observed_feature] -= (
                        curr_residual  # Obtain the model prediction
                    )
                    tmpDataDf.loc[curr_data_selection, observed_feature] += np.random.normal(
                        loc=0, scale=np.sqrt(curr_residual_var), size=curr_residual.shape[0]
                    )
                    tmp_list.append(
                        {
                            "Condition": condition,
        ...
        if (not np.any(areIcsVariedList)) or (
            varyICs == False
        ):  # Allow manual overwrite via varyICs, in case of only varying some of the ICs
            for observed_feature in model_to_observation_map.values():
                if n_conditions == 1:
                    ...
                    tmpDataDf.loc[tmpDataDf.Time == 0, observed_feature] = fitObj.data.loc[
                        fitObj.data.Time == 0, observed_feature
                    ].values
        ...
        if shuffle_params:
            for param in paramsToEstimateList:
                currParams[param].value = np.random.uniform(
                    low=currParams[param].min, high=currParams[param].max
                )
        # Fit
        currFitObj = minimize(
            residual_fun,
        ...
        if plot_bootstraps:
            ...
            for i, (variable, observed_feature) in enumerate(model_to_observation_map.items()):
                ...
                ax.plot(
                    tmpDataDf.Time,
                    tmpDataDf[observed_feature],
                    linestyle="",
                    marker="o",
                    linewidth=3,
                    color=plot_kws["palette"][bootstrapId],
                )
                if split_by is None:
                    ax.plot(
                        tmpDataDf.Time[np.isnan(tmpDataDf[observed_feature]) == False],
                        tmpDataDf.loc[
                            np.isnan(tmpDataDf[observed_feature]) == False, observed_feature
                        ]
                        - currFitObj.residual,
                        linewidth=3,
                        linestyle="-",
                    ...
                    for condition in tmpDataDf[split_by].unique():
                        ...
                        curr_index_range = data_to_residual_index_map[
                            (data_to_residual_index_map.Condition == condition)
                            & (data_to_residual_index_map.Feature == observed_feature)
                        ].Range.values[0]
                        curr_residual_scale = residual_scale.get(variable, 1.0)
                        curr_residual = (
                            currFitObj.residual[curr_index_range[0] : curr_index_range[1]]
                            * curr_residual_scale
                        )
                        curr_model_prediction = (
                            tmpDataDf.loc[curr_data_selection, observed_feature] - curr_residual
                        )
                        ax.plot(
                            curr_time,
                            curr_model_prediction,
    ...
    if plot_bootstraps:
        for i, (variable, observed_feature) in enumerate(model_to_observation_map.items()):
            ...
            if split_by is None:
                bestFitPrediction = (
                    fitObj.data.loc[
                        np.isnan(fitObj.data[observed_feature]) == False, observed_feature
                    ]
                    - fitObj.residual
                )
                ax.plot(
                    fitObj.data.Time[np.isnan(tmpDataDf[observed_feature]) == False],
                    bestFitPrediction,
                    linewidth=5,
                    linestyle="-",
                    color="k",
                ...
                for condition in fitObj.data[split_by].unique():
                    curr_data_selection = (np.isnan(fitObj.data[observed_feature]) == False) & (
                        fitObj.data[split_by] == condition
                    )
                    ...
                    curr_residual_scale = residual_scale.get(variable, 1.0)
                    curr_residual = (
                        fitObj.residual[curr_index_range[0] : curr_index_range[1]]
                        * curr_residual_scale
                    )
                    bestFitPrediction = (
                        fitObj.data.loc[curr_data_selection, observed_feature] - curr_residual
                    )
    ...
    if prior_experiment_df is not None:
        resultsDf = pd.concat([prior_experiment_df.drop("SSR", axis=1), resultsDf], axis=1)
    if outName is not None:
        resultsDf.to_csv(outName)
    return resultsDf


# ====================================================================================
def compute_confidenceInterval_prediction(
    fitObj,
    bootstrapResultsDf,
    alpha=0.95,
    ...
    atToProfile=None,
    at_kws={},
    initialConditionsDic=None,
    model_kws={},
    t_eval=None,
    n_time_steps=100,
    show_progress=True,
    returnTrajectories=False,
    estimate_fractions=False,
    **kwargs,
):
    # Initialise
    if t_eval is None:
        ...
        t_eval = (
            np.linspace(currPredictionTimeFrame[0], currPredictionTimeFrame[1], n_time_steps)
            if t_eval is None
            else t_eval
        )
    n_timePoints = len(t_eval)
    n_stateVars = len(create_model(fitObj.modelName, **model_kws).stateVars)
    treatmentScheduleList = (
        treatmentScheduleList
        if treatmentScheduleList is not None
        else utils.ExtractTreatmentFromDf(fitObj.data)
    )
    initialScheduleList = at_kws.get("initialScheduleList", [])
    if "initialScheduleList" in at_kws.keys():
        at_kws.pop(
            "initialScheduleList"
        )  # Remove the schedule as it is not an argument to the AT simulation function
    ...
    modelPredictionsMat_mean = np.zeros(
        (n_bootstraps, n_timePoints, n_stateVars + 2)
    )  # Array to hold model predictions for CI estimation
    modelPredictionsMat_indv = np.zeros(
        (n_bootstraps, n_timePoints, n_stateVars + 2)
    )  # Array to hold model predictions with residual variance for PI estimation
    for bootstrapId in tqdm(np.arange(n_bootstraps), disable=(show_progress == False)):
        ...
        if initialConditionsDic is not None:
            tmpModel.SetParams(**initialConditionsDic)
        if atToProfile is None:  # Do prediction on a fixed schedule
            tmpModel.Simulate(
                treatmentScheduleList=treatmentScheduleList, **kwargs.get("solver_kws", {})
            )
            ...
            if len(initialScheduleList) == 0:  # Simulate AT from t=0
                getattr(tmpModel, "Simulate_" + atToProfile)(
                    **at_kws, solver_kws=kwargs.get("solver_kws", {})
                )
            else:  # Simulate AT with a prior schedule
                tmpModel.Simulate(
                    treatmentScheduleList=initialScheduleList, **kwargs.get("solver_kws", {})
                )
        ...
        residual_variance_currEstimate = (
            bootstrapResultsDf["SSR"].iloc[bootstrapId] / fitObj.nfree
        )  # XXX Not sure this is correct for hierarchical model structure. Thus, PIs not used in paper XXX
        for stateVarId, var in enumerate(["TumourSize"] + tmpModel.stateVars):
            ...
            modelPredictionsMat_indv[bootstrapId, :, stateVarId] = tmpModel.resultsDf[
                var
            ].values + np.random.normal(
                loc=0, scale=np.sqrt(residual_variance_currEstimate), size=n_timePoints
            )
        modelPredictionsMat_mean[bootstrapId, :, -1] = tmpModel.resultsDf[
            "DrugConcentration"
        ].values  # Add separately as don't want to add this to the individual prediction matrix
    ...
    if initialConditionsDic is not None:
        tmpModel.SetParams(**initialConditionsDic)
    if treatmentScheduleList is None:
        treatmentScheduleList = utils.ExtractTreatmentFromDf(fitObj.data)
    if atToProfile is None:  # Do prediction on a fixed schedule
        tmpModel.Simulate(
            treatmentScheduleList=treatmentScheduleList, **kwargs.get("solver_kws", {})
        )
    ...
    for i, t in enumerate(t_eval):
        for stateVarId, var in enumerate(["TumourSize"] + tmpModel.stateVars):
            tmpDicList.append(
                {
                    "Time": t,
                    "Variable": var,
                    "Estimate_MLE": tmpModel.resultsDf[var].iloc[i],
                    "DrugConcentration": tmpModel.resultsDf["DrugConcentration"].iloc[i],
                    "CI_Lower_Bound": np.percentile(
                        modelPredictionsMat_mean[:, i, stateVarId], (1 - alpha) * 100 / 2
                    ),
                    "CI_Upper_Bound": np.percentile(
                        modelPredictionsMat_mean[:, i, stateVarId], (alpha + (1 - alpha) / 2) * 100
                    ),
                    "PI_Lower_Bound": np.percentile(
                        modelPredictionsMat_indv[:, i, stateVarId], (1 - alpha) * 100 / 2
                    ),
                    "PI_Upper_Bound": np.percentile(
                        modelPredictionsMat_indv[:, i, stateVarId], (alpha + (1 - alpha) / 2) * 100
                    ),
                }
            )
            if estimate_fractions and var != "TumourSize":
                ...
                bootstrap_fractions_list = (
                    modelPredictionsMat_mean[:, i, stateVarId]
                    / modelPredictionsMat_mean[:, i, stateVarId_totalSize]
                )
                tmpDicList[-1] = {
                    **tmpDicList[-1],
                    "Estimate_MLE_Fraction": tmpModel.resultsDf[var].iloc[i]
                    / tmpModel.resultsDf["TumourSize"].iloc[i],
                    "CI_Lower_Bound_Fraction": np.percentile(
                        bootstrap_fractions_list, (1 - alpha) * 100 / 2
                    ),
                    "CI_Upper_Bound_Fraction": np.percentile(
                        bootstrap_fractions_list, (alpha + (1 - alpha) / 2) * 100
                    ),
                }
    ...
    if returnTrajectories:
        ...
        for bootstrap_id in range(n_bootstraps):
            tmp_df = pd.DataFrame(
                modelPredictionsMat_mean[bootstrap_id],
                columns=["TumourSize", "S", "R", "DrugConcentration"],
            )
...
def benchmark_prediction_accuracy(
    ...
    **kwargs,
    ...
    for bootstrapId in tqdm(np.arange(n_bootstraps), disable=(show_progress == False)):
        ...
        # Save results
        tmpDicList.append(
            {"Model": fitObj.modelName, "BootstrapId": bootstrapId, "rSquared": r2Val}
        )
    return pd.DataFrame(tmpDicList)


# ====================================================================================
def compute_confidenceInterval_parameters(
    fitObj, bootstrapResultsDf, paramsToEstimateList=None, alpha=0.95
):
    # Initialise
    if paramsToEstimateList is None:
        paramsToEstimateList = [
            param for param in fitObj.params.keys() if fitObj.params[param].vary
        ]
    ...
    for i, param in enumerate(paramsToEstimateList):
        tmpDicList.append(
            {
                "Parameter": param,
                "Estimate_MLE": fitObj.params[param].value,
                "Lower_Bound": np.percentile(
                    bootstrapResultsDf[param].values, (1 - alpha) * 100 / 2
                ),
                "Upper_Bound": np.percentile(
                    bootstrapResultsDf[param].values, (alpha + (1 - alpha) / 2) * 100
                ),
            }
        )
    return pd.DataFrame(tmpDicList)
...
def test_model_on_well(
    ...
    curr_data_df = prepare_data(
        data_df,
        specs_dic={"WellId": well_id},
    ...
    if atToProfile is None:
        ...
        for passage_id in np.arange(0, n_passages):
            ...
            # Add the density
            curr_seeding_density = curr_passage_df[
                curr_passage_df.Time == t_start
            ].Count_Total.mean()
            seeding_density_list.append(curr_seeding_density)

        # Package up to feed to the ODE solver
        at_kws = {
            "seeding_density": seeding_density_list,  # np.mean(seeding_density_list),
            "treatment_schedule": treatment_schedule_list,
        }

    # Simulate
    if n_time_steps is None and t_eval is None:
        n_time_steps = 2 * (
            int(curr_data_df.Time.max()) + 1
        )  # Set the number of time steps to be twice the number of time points in the data
    initial_conditions_dic = {
        "S0": curr_data_df["Count_Sensitive"].iloc[0],
        "R0": curr_data_df["Count_Resistant"].iloc[0],
    }
    predictions_df, trajectories_df = compute_confidenceInterval_prediction(
        fitObj=fit_obj,
        ...
        estimate_fractions=True,
        show_progress=show_progress,
        t_eval=t_eval,
        n_time_steps=n_time_steps,
        alpha=significance_level_ci,
        solver_kws=solver_kws,
        optimiser_kws=optimiser_kws,
    )
    predictions_df = predictions_df.rename(
        columns={
            "Estimate_MLE": "Estimate_MLE_Count",
    ...
    predictions_df["PassageId"] = predictions_df["Time"].apply(
        lambda x: np.max(np.where(np.float64(x) >= passage_start_time_list)[0])
    )

    # Save mle and bootstrap trajectories
    trajectories_df.reset_index(drop=True, inplace=True)
    mle_df = predictions_df.pivot(
        index=["Time", "DrugConcentration"], columns="Variable", values="Estimate_MLE_Count"
    )
    ...
    trajectories_df["PassageId"] = trajectories_df["Time"].apply(
        lambda x: np.max(np.where(np.float64(x) >= passage_start_time_list)[0])
    )
