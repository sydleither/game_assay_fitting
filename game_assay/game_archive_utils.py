...
def load_spatial_data(row_df, data_dir):
    ...
    if len(row_df) > 1:
        print(row_df)
        raise ValueError("Too many rows in dataframe passed to load_spatial_data()")
    tmp_list = []
    row_df = row_df.reset_index().iloc[0]
    for i, file in enumerate(row_df[["Locations Cell Type 1", "Locations Cell Type 2"]]):
        ...
        tmp_df.rename(
            columns={
                "ObjectNumber": "CellId",
                "Metadata_Timepoint": "Time_index",
                "Metadata_Well": "WellId",
            },
            inplace=True,
        )
        tmp_df["Time_hours"] = (tmp_df["Time_index"] - 1) * row_df["Imaging Frequency"]
        tmp_df["CellType"] = cell_type + "-" + row_df["Fluorophore %d" % population_id].lower()
        tmp_df["Name"] = row_df["Name"]
