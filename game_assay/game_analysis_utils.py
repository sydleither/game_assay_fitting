...
def plot_data(dataDf, x="Time", y='Confluence', style=None, hue=None, estimator=None,
    ...
    if plot_drug:
        plot_drug_bar(dataDf, ax, timeColumn=x, treatmentColumn=treatment_column, treatment_notation_mode=treatment_notation_mode, 
                      plotDrugAsBar=True, drugBarPosition=drug_bar_position, drugBarColour=drug_colour, y2lim=y2lim)
...
# ---------------------------------------------------------------------------------------------------------------
def plot_drug_bar(drug_data_df, ax, timeColumn="Time", treatmentColumn="DrugConcentration", treatment_notation_mode="post",
                  plotDrugAsBar=True, drugBarPosition=0.85, y2lim=None, alpha=1., drugBarColour="black", decorateY2=False, zorder=1, **kwargs):
    """
    ...
    if plotDrugAsBar:
        ...
        max_height = 1 - drugBarPosition # Max height of the bar (above the drugBarPosition) relative to the y2 axis
        ...
        ax2.fill_between(drug_data_df[timeColumn], drugBarPosition, drugConcentrationVec,
                            step="post", color=drugBarColour, alpha=alpha, label="Drug Concentration", zorder=zorder)
        ax2.axis("off")
    else:
        pass
